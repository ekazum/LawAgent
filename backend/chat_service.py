"""Chat streaming: builds the SSE event stream for /api/chat.

Pure logic (no FastAPI routing) — the route handler in app.py wraps
chat_stream() in a StreamingResponse.
"""
...
def file_content_block(file: FileInput) -> dict:
...
def chat_stream(req: ChatRequest, api_key: str, owner: str) -> Iterator[str]:
    ...
    for attached in req.all_files:
        try:
            user_blocks.append(file_content_block(attached))
